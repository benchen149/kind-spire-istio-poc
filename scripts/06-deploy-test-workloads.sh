...
kubectl label namespace istio-validation spiffe-managed=true istio-injection=enabled --overwrite
...
kubectl wait pod -n istio-validation -l app=payment-gateway --for=condition=ready --timeout=120s
kubectl wait pod -n istio-validation -l app=payment-core --for=condition=ready --timeout=120s
...
POD=$(kubectl get pod -n istio-validation -l app=payment-gateway -o jsonpath='{.items[0].metadata.name}')
istioctl proxy-config secret -n istio-validation "$POD"

echo "=== 驗證 mTLS + AuthorizationPolicy（允許的呼叫應為 200）==="
kubectl exec -n istio-validation deploy/payment-gateway -c app -- \
  curl -sS -o /dev/null -w "gateway->core: HTTP %{http_code}\n" \
  http://payment-core.istio-validation.svc.cluster.local/get --max-time 5
